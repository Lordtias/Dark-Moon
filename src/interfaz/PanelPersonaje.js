...
const DETALLES = Object.freeze({
  ...
  precision:{etiqueta:"Precisión",icono:"◎",resolucion:"precision"}, dispersion:{etiqueta:"Dispersión",icono:"⌁",resolucion:"dispersion",porcentaje:true}, "penetracion-armadura":{etiqueta:"Penetración de Armadura",icono:"➶",resolucion:"penetracionArmadura",porcentaje:true}, evasion:{etiqueta:"Evasión",icono:"↗",resolucion:"evasion"}, armadura:{etiqueta:"Armadura",icono:"⬙",resolucion:"armadura"},
  ...
  "ajuste-comercial":{etiqueta:"Ajuste comercial",icono:"¤",resolucion:"ajusteComercial",porcentaje:true,escala:100},
  "hallazgo-magico":{etiqueta:"Hallazgo mágico",icono:"✦",resolucion:"hallazgoMagico",porcentaje:true,limiteDominio:100},
});
...
export class PanelPersonaje {
  ...
  crearContenido() {
    ...
    this.crearCombateAvanzado(); this.crearSuerte(); this.crearHabilidades(); this.crearPasivas(); this.crearEfectos(); this.configurarDesgloses();
  }
  ...
  crearCombateAvanzado(){const r=this.contenedor.querySelector('[data-seccion-personaje="combate"] .resumen-personaje');if(!r)return;r.append(this.crearDato(traducir("interfaz.personaje.dispersion",{respaldo:"Dispersión"}),"dispersion","0%"),this.crearDato(traducir("interfaz.personaje.penetracionArmadura",{respaldo:"Penetración de Armadura"}),"penetracion-armadura","0%"));}
  crearSuerte(){const r=this.contenedor.querySelector('[data-seccion-personaje="combate"] .resumen-personaje');if(!r)return;r.append(this.crearDato(traducir("interfaz.personaje.ajusteComercial",{respaldo:"Ajuste comercial"}),"ajuste-comercial","+0%"),this.crearDato(traducir("interfaz.personaje.hallazgoMagico",{respaldo:"Hallazgo mágico"}),"hallazgo-magico","+0%"));}
  ...
  actualizarEstadisticas(player,e){this.asegurarMental();const v={precision:e.precision,dispersion:`${formato(e.dispersion)}%`,"penetracion-armadura":`${formato(e.penetracionArmadura)}%`,evasion:e.evasion,armadura:e.armadura,critico:`${formato(e.probabilidadCritico)}%`,bloqueo:`${formato(e.probabilidadBloqueo)}%`,"mitigacion-bloqueo":`${formato(e.mitigacionBloqueo)}%`,"regen-vida":formato(e.regeneracionVida),"regen-mana":formato(e.regeneracionMana),percepcion:formato(Math.max(0,this.resolucionPercepcionActual.resultado)),alcance:this.resolucionAlcanceActual?.resultado??player.alcanceAtaque,"danio-magico":`×${e.multiplicadorDanioMagico.toFixed(2)}`,"potencia-habilidad":signoPorcentaje(this.potenciaActual?.potenciaHabilidad??0),"potencia-efectos":signoPorcentaje(e.potenciaEfectos),"res-fuego":`${formato(e.resistencias.fuego)}%`,"res-frio":`${formato(e.resistencias.frio)}%`,"res-rayo":`${formato(e.resistencias.rayo)}%`,"res-veneno":`${formato(e.resistencias.veneno)}%`,"res-congelamiento":`${formato(e.resistenciasEfectos.congelamiento)}%`,"res-aturdimiento":`${formato(e.resistenciasEfectos.aturdimiento)}%`,"res-envenenamiento":`${formato(e.resistenciasEfectos.envenenamiento)}%`,"res-quemadura":`${formato(e.resistenciasEfectos.quemadura)}%`,"res-mental":`${formato(e.resistenciaMental)}%`,"ajuste-comercial":signoPorcentaje(e.ajusteComercial*100),"hallazgo-magico":signoPorcentaje(e.hallazgoMagico)};for(const [k,val] of Object.entries(v))this.obtener(`[data-personaje="${k}"]`).textContent=val;}
  ...
  detalleEstadistica(k){
    ...
    if(k==="danio-medio"){
      ...
      for(const componente of this.estadisticasActuales?.danioFisico?.componentes??[]){
      ...
      return{
        titulo:m.etiqueta,icono:m.icono,valorFinal:visible,descripcion:descripcionDetalle(k),filas,
        nota:traducir("interfaz.personaje.desgloseDanioFuenteNota",{respaldo:"Los modificadores de fuente mostrados provienen de la misma resolución canónica utilizada para calcular el daño actual."}),
      };
    }
    if(k==="dps")return{
      titulo:m.etiqueta,
    ...
    const filas=[];
    if(r){
      filas.push({tipo:"base",etiqueta:traducir("interfaz.personaje.valorBase",{respaldo:"Valor base"}),valor:valorResolucion(r.valorBase,m.porcentaje,m.escala??1)});
      ...
      for(const mod of r.desglose?.aplicados??[])filas.push({tipo:tipoOperacion(mod.operacion,mod.valor),etiqueta:this.nombreFuente(mod),valor:formatearMod(mod,m.escala??1,m.porcentaje===true)});
...
function formatearMod(m,escala=1,forzarPorcentaje=false){const original=Number(m?.valor)||0,v=original*escala,s=v>0?"+":"";if([OPERACIONES_MODIFICADOR.PORCENTAJE_BASE,OPERACIONES_MODIFICADOR.PORCENTAJE_TOTAL,OPERACIONES_MODIFICADOR.PORCENTAJE_MULTIPLICATIVO,OPERACIONES_MODIFICADOR.PORCENTAJE_INVERSO].includes(m?.operacion))return`${original>0?"+":""}${original}%`;if([OPERACIONES_MODIFICADOR.MULTIPLICAR,OPERACIONES_MODIFICADOR.MULTIPLICAR_REDONDEAR].includes(m?.operacion))return`×${original}`;if(m?.operacion===OPERACIONES_MODIFICADOR.LIMITAR_MAXIMO)return forzarPorcentaje?`máx. ${formato(v)}%`:`máx. ${formato(v)}`;return forzarPorcentaje?`${s}${formato(v)}%`:`${s}${formato(v)}`;}
function descripcionDetalle(clave){
  const fallback={
    ...
    "atributo:constitucion":"Atributo de resistencia corporal y presencia. Aporta Vida máxima, regeneración de Vida, Potencia de Aura y defensas frente a Veneno y estados perjudiciales.",
    ...
    "atributo:suerte":"Atributo de fortuna. Ajusta los precios comerciales y aumenta el peso relativo de rarezas superiores a Común al materializar botín.",
    ...
    dispersion:"Pérdida máxima de Precisión a larga distancia. Se aplica progresivamente después de la mitad del alcance; cuanto más cerca de 0%, mejor.",
    "penetracion-armadura":"Puntos porcentuales que se restan a la mitigación producida por la Armadura. El excedente puede generar vulnerabilidad física.",
    ...
    "ajuste-comercial":"Modificador canónico de precios derivado de Suerte. Los comerciantes consumen este resultado sin recalcular Suerte.",
    "hallazgo-magico":"Aumenta el peso relativo de rarezas superiores a Común cuando el botín se materializa. No cambia cantidad, Tier ni presupuesto.",
  };
...
function valorResolucion(v,p,escala=1){const s=formato(Number(v)*escala);return p&&s!=="—"?`${s}%`:s;}
