...
const PROPIEDADES_ACTIVAS = new Set([
  ...
  "danioFuegoLocalMinimo",
  "danioFuegoLocalMaximo",
  "danioFrioLocalMinimo",
  "danioFrioLocalMaximo",
  "danioRayoLocalMinimo",
  "danioRayoLocalMaximo",
  "danioVenenoLocalMinimo",
  "danioVenenoLocalMaximo",
  "armadura",
  "vidaMaxima",
  "manaMaximo",
  "precision",
...
function validarReglasGeneracion(reglas) {
  ...
  validarObjetoConfiguracion(
    configuracionNivel,
    "La configuración del nivel de objeto",
  );
  validarTexto(
  ...
  for (const entrada of configuracionNivel.distribucion) {
    validarObjetoConfiguracion(
      entrada,
      "Una entrada de la distribución del nivel de objeto",
    );
    ...
    if (desplazamientos.has(entrada.desplazamiento)) {
      throw new Error(
        `La distribución del nivel repite el desplazamiento ${entrada.desplazamiento}.`,
      );
    }
...
function validarCatalogoRarezas(rarezas) {
  ...
  for (const [idRareza, rareza] of Object.entries(rarezas)) {
    ...
    if (
      ...
      throw new Error(
        `La rareza "${idRareza}" necesita un color hexadecimal de seis dígitos.`,
      );
    }
    ...
    if (typeof rareza.generaAfijosAleatorios !== "boolean") {
      throw new Error(
        `La rareza "${idRareza}" debe indicar si genera afijos aleatorios.`,
      );
    }
    ...
    if (rareza.afijosMaximos < rareza.afijosMinimos) {
      throw new Error(
        `La rareza "${idRareza}" tiene un máximo de afijos menor que su mínimo.`,
      );
    }
    if (rareza.prefijosMaximos + rareza.sufijosMaximos < rareza.afijosMaximos) {
      throw new Error(
        `La rareza "${idRareza}" no tiene suficientes espacios de prefijo y sufijo.`,
      );
    }
    if (
      ...
      throw new Error(
        `El nivel mínimo de la rareza "${idRareza}" debe ser un entero mayor o igual que uno.`,
      );
    }
...
function validarDistribucionCantidadAfijos(idRareza, rareza) {
  ...
  if (!Array.isArray(distribucion) || distribucion.length === 0) {
    throw new Error(
      `La rareza "${idRareza}" necesita una distribución de cantidad de afijos.`,
    );
  }
  ...
  for (const entrada of distribucion) {
    validarObjetoConfiguracion(
      entrada,
      `Una cantidad de afijos de la rareza "${idRareza}"`,
    );
    if (
      ...
      throw new Error(
        `La rareza "${idRareza}" contiene la cantidad de afijos inválida ${entrada.cantidad}.`,
      );
    }
    if (cantidades.has(entrada.cantidad)) {
      throw new Error(
        `La rareza "${idRareza}" repite la cantidad ${entrada.cantidad} en su distribución.`,
      );
    }
  ...
  if (
    ...
    throw new Error(
      `La rareza "${idRareza}" no genera afijos aleatorios y solamente puede configurar la cantidad cero.`,
    );
  }
}
...
function validarCatalogoAfijos({
  ...
  for (const [idAfijo, afijo] of Object.entries(catalogo)) {
    ...
    if (idsAfijos.has(idAfijo)) {
      throw new Error(
        `El afijo "${idAfijo}" está repetido entre los catálogos de prefijos y sufijos.`,
      );
    }
    ...
    for (const idRareza of afijo.rarezasPermitidas) {
      if (!idsRarezas.has(idRareza)) {
        throw new Error(
          `El afijo "${idAfijo}" referencia la rareza inexistente "${idRareza}".`,
        );
      }
    }
    if (
...
function validarAplicacionAfijo(idAfijo, aplicaA) {
  ...
  validarListaTextos(
    aplicaA.tipos,
    `tipos compatibles del afijo "${idAfijo}"`,
    false,
  );
  ...
  for (const tipoObjeto of aplicaA.tipos) {
    if (!TIPOS_OBJETO_COMPATIBLES.has(tipoObjeto)) {
      throw new Error(
        `El afijo "${idAfijo}" utiliza el tipo de objeto no reconocido "${tipoObjeto}".`,
      );
    }
  }

  validarListaTextos(
    aplicaA.ranurasIncluidas,
    `ranuras incluidas del afijo "${idAfijo}"`,
    true,
  );
  validarListaTextos(
    aplicaA.ranurasExcluidas,
    `ranuras excluidas del afijo "${idAfijo}"`,
    true,
  );
  validarListaTextos(
    aplicaA.familiasIncluidas ?? [],
    `familias incluidas del afijo "${idAfijo}"`,
    true,
  );
  validarListaTextos(
    aplicaA.familiasExcluidas ?? [],
    `familias excluidas del afijo "${idAfijo}"`,
    true,
  );
}
...
function validarGradosAfijo(idAfijo, afijo) {
  ...
  for (const grado of afijo.grados) {
    ...
    if (!Number.isInteger(grado.grado) || grado.grado < 1) {
      throw new Error(`Existe un grado inválido en el afijo "${idAfijo}".`);
    }
    if (registrados.has(grado.grado)) {
      throw new Error(`El afijo "${idAfijo}" repite el grado ${grado.grado}.`);
    }
    registrados.add(grado.grado);

    if (
      !Number.isInteger(grado.nivelObjetoMinimo) ||
      grado.nivelObjetoMinimo < 1
    ) {
      throw new Error(
        `El grado ${grado.grado} de "${idAfijo}" necesita un nivel mínimo válido.`,
      );
    }
    validarEnteroNoNegativo(
      grado.peso,
    ...
    validarObjetoConfiguracion(
      grado.valores,
      `Los valores del grado ${grado.grado} de "${idAfijo}"`,
    );
    const configuradas = Object.keys(grado.valores);

    for (const propiedad of propiedadesEfecto) {
      if (!configuradas.includes(propiedad)) {
        throw new Error(
          `El grado ${grado.grado} de "${idAfijo}" no define valores para "${propiedad}".`,
        );
      }
    }
    for (const propiedad of configuradas) {
      if (!propiedadesEfecto.includes(propiedad)) {
        throw new Error(
          `El grado ${grado.grado} de "${idAfijo}" contiene la propiedad adicional "${propiedad}".`,
        );
      }
      validarRangoGrado(
        idAfijo,
...
function validarRangoGrado(idAfijo, numeroGrado, propiedad, rango) {
  ...
  if (
    ...
    throw new Error(
      `El rango "${propiedad}" del grado ${numeroGrado} de "${idAfijo}" no es válido.`,
    );
  }
...
function validarListaTextos(lista, descripcion, permitirVacia) {
  ...
  for (const valor of lista) {
    ...
    if (normalizados.has(normalizado)) {
      throw new Error(
        `La configuración de ${descripcion} contiene el valor repetido "${normalizado}".`,
      );
    }
