...
`OrganizadorArbolHabilidades` se aplica a todas las maestrías y habilidades sin distinguir magia, armas o armaduras. El eje vertical deriva exclusivamente de `requisitoNivelMaestria`. La conectividad real del grafo determina el reparto horizontal: nodos con predominio de salidas, entradas o sin conexiones reciben carriles relativos diferentes, y los nodos que comparten nivel ocupan carriles libres próximos. No existen posiciones por ID, ramas especiales para maestrías físicas ni un eje artificial para completar visualmente árboles todavía escasos.

Las conexiones pueden provenir de relaciones inferibles desde modificadores canónicos o del contrato declarativo `relacionesArbol`, disponible para cualquier maestría. `modificacion` usa línea continua y significa que el efecto está dirigido específicamente a la habilidad destino. `sinergia` usa línea punteada y significa que la habilidad destino se beneficia por una estadística, estado o contexto compartido. Ambos expresan interacción real y **nunca** un requisito de aprendizaje. El validador exige destino existente, evita autorrelaciones/duplicados y no permite cruces de maestría sin un futuro contrato transversal aprobado. Arcos utiliza el mismo organizador con 17 sinergias declaradas y ningún CSS/JS específico de esa maestría.

Cada nodo muestra exclusivamente el icono y el grado `actual/máximo`. Las habilidades sin aprender usan menor opacidad; las bloqueadas por nivel se atenúan más. Las conexiones parten/llegan al borde del nodo para que las redes densas sean legibles y se recalculan cuando cambia la geometría del lienzo. La ventana de Habilidades usa casi todo el viewport disponible, pero si el árbol supera el alto real debe habilitar desplazamiento vertical en vez de recortar niveles o miniaturizar los nodos. Los iconos se presentan a mayor tamaño sin alterar los archivos fuente.
