...
69. `modificacion` en el árbol representa una modificación específica de la habilidad destino y usa línea continua; `sinergia` representa un beneficio por estadística, estado o contexto compartido y usa línea punteada.
70. una habilidad de arma con `usaAlcanceArma=true` consume `Combatiente.alcanceAtaque`; no se duplica el mismo concepto como `ATRIBUTO_HABILIDAD.ALCANCE`.
71. el nivel de maestría es el único eje vertical del árbol; la conectividad real puede modificar únicamente la distribución horizontal y nunca inventa requisitos o ramas.
72. Arcos posee 17 sinergias declaradas con el contenido actual: doce desde sus cuatro pasivas generales hacia los tres disparos, tres desde `Aura de Precisión` y dos desde `Francotirador`.
...
Para Arcos, AR1.1 dejó declaradas las relaciones entre pasivas generales de arma y disparos, además de las sinergias de `Francotirador`. AR1.2 corrige la semántica visual: los modificadores de Precisión, daño de fuente, Crítico y Alcance de ataque son generales a la familia Arco y por eso sus doce enlaces son `sinergia`, no `modificacion`. `Aura de Precisión` agrega otras tres sinergias con los disparos ofensivos; `Francotirador` conserva sus dos enlaces con `Disparo potente` y `Disparo evasivo`. El total actual de Arcos es 17 relaciones punteadas.
...
## Integración AR1.2 — Alcance de arma y grafo legible

Cuando `usaAlcanceArma=true`, `ConfiguracionHabilidadEfectiva` toma el valor ya resuelto de `Combatiente.alcanceAtaque`. Este contrato conserva una sola fuente para alcance de arma: arma equipada → `OBJETIVOS_MODIFICADOR.ALCANCE_ATAQUE` → getter del combatiente → habilidad. No se crea un segundo `atributoHabilidad.alcance` para representar una estadística que ya existe fuera de la habilidad. Las habilidades que poseen alcance propio continúan resolviéndolo mediante `ATRIBUTOS_HABILIDAD.ALCANCE`.

El organizador conserva `requisitoNivelMaestria` como posición vertical y calcula la posición horizontal desde la conectividad entrante/saliente. El algoritmo es común a todas las maestrías y no consulta IDs, nombres, tipo de arma ni categoría. Las filas con varios nodos asignan carriles libres cercanos a su posición preferida. El panel utiliza esos valores relativos para ubicar los nodos, conecta bordes de los iconos mediante SVG y vuelve a dibujar cuando cambia el tamaño real del lienzo mediante `ResizeObserver`. Si el árbol supera el alto disponible, el propio árbol es la superficie desplazable y no recorta niveles.
