...
### V-039 — Ruta contextual y detalle de habilidades

`OrganizadorArbolHabilidades` se aplica a todas las maestrías y habilidades sin distinguir magia, armas o armaduras, pero no dibuja un grafo. Ordena las habilidades exclusivamente por `requisitoNivelMaestria` y devuelve niveles legibles para una ruta vertical de tarjetas. No calcula posiciones, carriles horizontales, SVG ni geometría por contenido; conserva su nombre público para no crear otra capa de presentación.

Cada tarjeta muestra icono, nombre, tipo, grado `actual/máximo` y estado. Las habilidades sin aprender mantienen menor opacidad y las bloqueadas por nivel se atenúan más. La ruta es la superficie vertical desplazable; debe conservar su posición mientras el jugador inspecciona otra habilidad. En escritorio, el clic marca la tarjeta y actualiza únicamente una ficha contextual fija a la derecha: no reconstruye la ruta ni devuelve el scroll al inicio. La ventana se limita al ancho de contenido útil para no reservar un marco lateral vacío.

Solo se representan relaciones directas de `modificacion`: una pasiva modifica específicamente a la habilidad destino cuando sus datos canónicos declaran `condiciones.idHabilidad`, o cuando existe una relación declarativa del mismo tipo. Se presentan junto a las tarjetas mediante chips `Modifica directamente` y `Mejorada por`; un chip selecciona su habilidad relacionada. No existen líneas, flechas, SVG ni relaciones visuales `sinergia` para atributos generales, estados o contextos compartidos. Las relaciones son informativas y **nunca** un requisito de aprendizaje. Retirar una relación visual no altera el modificador canónico que ya aplica el motor.

La ficha se deriva del contenido y se divide en `Pasiva`, `Aura`, `Maldición` u `Ofensiva`; solo aparecen campos pertinentes. Las Auras no muestran daño inexistente, las Maldiciones exponen probabilidad/duración/resistencia cuando corresponde, las Ofensivas pueden exponer daño directo, efectos o zonas, y las Pasivas muestran sus modificadores y ámbito de aplicación. Las activas leen valores efectivos desde `ConfiguracionHabilidadEfectiva`. Aprender, mejorar y gestionar la barra continúan reutilizando los sistemas existentes, sin replicar reglas de progresión.
...
La ruta de habilidades prioriza tarjetas táctiles legibles y una única superficie vertical desplazable antes que mostrar más información simultáneamente. En móvil la navegación de categorías se compacta en una banda superior y las maestrías de la categoría activa se presentan en una banda horizontal desplazable. La ficha fija de escritorio no se duplica: tocar una tarjeta abre el detalle en la capa de acción existente. De este modo categorías con muchas maestrías, como Armas, no consumen verticalmente el viewport antes de la ruta ni crean zonas táctiles superpuestas.


Todo panel de contenido debe conservar acceso a la información cuando su contenido excede el alto disponible: nunca se acepta `overflow:hidden` como solución si recorta datos jugables. En escritorio pueden mantenerse scrolls internos existentes cuando resultan útiles. Los modales y paneles fullscreen móviles deben tener **una única superficie vertical desplazable principal**; listas, fichas de detalle y pies de acciones no pueden crear zonas táctiles aisladas que impidan continuar el mismo swipe al cambiar de sección.
...
`Disparo múltiple` debe sentirse como una única ráfaga continua: sus 2–4 proyectiles se escalonan dentro de la misma ejecución con intervalos breves y perceptibles. Las flechas pueden separar levemente su origen y llegada **solo en presentación** para evitar superposición y reforzar la lectura tipo rapid-fire; cada proyectil conserva intacto el objetivo, impacto, crítico y daño que recibió del dominio. Esa separación visual no crea Dispersión adicional ni cambia casillas alcanzadas.

`Disparo potente` utiliza la flecha real de la munición y agrega una estela luminosa **adherida al proyectil** durante la trayectoria, con lectura de traza/cometa más marcada que un disparo normal. La cola viaja con la flecha y se desvanece con ella; no se representa como puntos estáticos distribuidos por toda la trayectoria. El efecto debe transmitir mayor impulso sin convertirse en una habilidad elemental de Fuego ni ocultar la cuadrícula.
...
`Disparo evasivo` admite un enemigo o una casilla libre válida. El proyectil viaja hacia la casilla elegida y el actor inicia al mismo tiempo el salto corto hacia el destino táctico ya resuelto en dirección opuesta. La concurrencia pertenece únicamente al plan visual: dominio conserva el orden disparo → impacto/muerte cuando existe objetivo → desplazamiento. La animación es un arco bajo y rápido, no un salto sobrenatural; la sombra acompaña el desplazamiento y recupera su escala original al aterrizar.
...
### V-044 — Disponibilidad de barra y feedback temporal de rechazo

La barra rápida puede atenuar una habilidad cuando el sistema canónico informa que no está disponible por Maná, equipamiento, munición u otro requisito ya resuelto. La presentación no vuelve a leer el inventario ni interpreta familias de arma para decidir disponibilidad. La ranura conserva su icono, tecla y contexto; el estado apagado comunica indisponibilidad actual, no desasigna la habilidad.
