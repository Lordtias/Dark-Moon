...
La regla para escalar la magnitud de un descriptor según su operación también pertenece a `ContratosModificadoresCombatiente.js`. Potencia de Efectos y acumulaciones temporales reutilizan esa misma semántica en vez de mantener interpretaciones paralelas.
...
`ProgresoHabilidadesJugador.js` es la única fuente de verdad para niveles, XP, puntos y grados. `SistemaExperienciaMaestrias.js` es la única ruta productiva que traduce hechos ya resueltos del juego a XP sin recalcular combate: Maná realmente consumido para magia, daño real aplicado para armas, mitigación real de Armadura para categorías defensivas y mitigación de Bloqueo para Escudos. `Player` no expone un atajo productivo para sumar XP directamente. El depurador conserva una inyección explícita de XP únicamente para preparar pruebas.
...
HP4 incorporó además 28 activas de soporte: cuatro auras elementales, doce maldiciones y doce auras físicas. Auras y maldiciones se implementan sobre `SistemaEfectosTemporales`; las auras funcionan como emisiones móviles ligadas al emisor y no como zonas estáticas. No existen `SistemaAuras` ni `SistemaMaldiciones` paralelos.

Las cuatro maestrías mágicas conservan sus ofensivas base y amplían el árbol con pasivas, aura elemental y maldiciones. Ejemplos de la línea ofensiva original:
...
src/interfaz/habilidades/ClasificadorPresentacionHabilidades.js
...
- `ConfiguracionHabilidadEfectiva`: valores numéricos efectivos de una activa, resueltos desde el centralizador común.
...
`Efectos.json` contiene actualmente 35 definiciones reutilizables. Incluye estados clásicos como ralentización, electrización, congelamiento, aturdimiento, envenenamiento, quemadura y parálisis, además de los efectos utilizados por auras y maldiciones. Silencio es funcional y usa el bloqueo específico de habilidades.

Los efectos numéricos de pasivas temporales, auras y maldiciones exponen descriptores al mismo `SistemaModificadoresCombatiente`; `SistemaEfectosTemporales` conserva únicamente duración, renovación/acumulación, emisiones y ciclo de vida. Las acumulaciones escalan magnitudes mediante la semántica común declarada en `ContratosModificadoresCombatiente.js`.
...
`Resistencia Mental` también se presenta como estadística derivada y reduce la probabilidad de aplicación de Maldiciones; su valor base deriva de Sabiduría, pasa por el centralizador y se limita al rango definido por el contrato.

Para los estados con resistencia propia, la resistencia decide si el efecto se aplica. Las inmunidades pueden impedir la aplicación y retirar efectos activos cuando la configuración así lo exige.
...
Las rutas pueden aparecer en JSON. `Habilidades.json.icono` es el contrato de imagen de habilidades y los 104 iconos actuales están normalizados físicamente a 128×128. No se debe mover o renombrar una imagen sin actualizar y validar todas sus referencias.
...
- Árbol de habilidades y maestrías.
- Aprendizaje y mejora.
- Barra de diez ranuras.
- Habilidades activas y pasivas; Auras y Maldiciones.
- Consumo de Maná.
- Potencia de habilidad.
- Daño elemental.
- Efectos, Auras y Maldiciones.
- Resistencias, Resistencia Mental e inmunidades.
- Zonas temporales.
